...
**↳ Ответ:** Стрим имеет overhead: создание объектов `Spliterator`, `Sink`, лямбда-классов, вызов виртуальных методов через цепочку `Sink`. На коллекции из 5-10 элементов этот overhead может быть сравним или больше полезной работы. Цикл `for` — прямой bytecode, без лишних объектов. Практическое правило: стрим — для читаемости и сложных пайплайнов; цикл — когда нужна максимальная скорость на маленьких данных или когда нужно break/continue/return из середины.
...
**↳ Ответ:** Stateful: `distinct()`, `sorted()`, `limit()`, `skip()` — они должны помнить предыдущие элементы или всю коллекцию. `distinct` хранит `HashSet` виденных элементов, `sorted` материализует весь стрим. В parallel stream это проблема: stateful-операции требуют глобального барьера — все потоки должны синхронизироваться, что частично или полностью убивает параллелизм. Stateless-операции (`filter`, `map`, `flatMap`) параллелятся идеально — каждый элемент независим.
...
**↳ Ответ:** Да. Правило: filter раньше map. Если filter отсеивает 90% элементов, map вызовется только на 10% — меньше работы. Если map стоит первым, трансформация (возможно, дорогая) делается для всех элементов, включая те, что будут отфильтрованы дальше. Пример: `users.stream().map(this::fetchFullProfile).filter(u -> u.isActive())` — `fetchFullProfile` вызывается для всех, в том числе неактивных. Порядок: сначала дешёвый filter, потом дорогой map.
...
**↳ Ответ:** Используйте `mapToInt/mapToDouble` всегда при работе с числами в терминальных агрегациях — избегаете boxing/unboxing `Integer/Double`. `IntStream` дополнительно предоставляет `sum()`, `average()`, `min()`, `max()`, `summaryStatistics()` — без необходимости в `reduce` или `Collectors`. Правило: если после map идёт числовая агрегация — всегда `mapToInt/mapToLong/mapToDouble`; если дальше снова нужен `Stream<T>` — обычный `map`.
...
**↳ Ответ:** `Stream.toList()` (Java 16+) возвращает **неизменяемый** список — попытка добавить элемент бросит `UnsupportedOperationException`. `Collectors.toList()` возвращает изменяемый `ArrayList` (по контракту «изменяемый», хотя документально это не жёстко). Если нужно добавить элементы после сбора — используйте `Collectors.toList()`. Для read-only данных — `Stream.toList()` предпочтительнее: короче, явно неизменяем, чуть быстрее (нет копирования в `ArrayList`).
...
**↳ Ответ:** Реализовать интерфейс `Collector<T, A, R>` с 5 методами: `supplier()` — создаёт пустой аккумулятор (`() -> new ArrayList<>()`); `accumulator()` — добавляет элемент в аккумулятор (`(list, el) -> list.add(el)`); `combiner()` — объединяет два аккумулятора для parallel (`(l1, l2) -> { l1.addAll(l2); return l1; }`); `finisher()` — финальное преобразование (`Function.identity()` если аккумулятор = результат); `characteristics()` — флаги (`UNORDERED`, `IDENTITY_FINISH`, `CONCURRENT`). Проще — `Collector.of(supplier, accumulator, combiner, finisher, characteristics)`.
...
**↳ Ответ:** `Optional.stream()` возвращает `Stream` из нуля или одного элемента — пустой, если `Optional.empty()`, и из одного значения, если присутствует. `flatMap(Optional::stream)` «разворачивает» каждый `Optional` в этот мини-стрим и все их конкатенирует — в итоге получается стрим только непустых значений. Это лучше filter + map, потому что не требует промежуточного `Optional::isPresent` + `Optional::get` (который считается антипаттерном без `isPresent` проверки), читается как одно семантическое действие, и не создаёт промежуточный `Stream<Optional<T>>`.
...
**↳ Ответ:** В функциональном программировании это называется `bind` или `>>=` (monadic bind) из теории монад. `flatMap` — это операция монады: она берёт «обёрнутое значение» (`Stream<T>`, `Optional<T>`, `List<T>`) и функцию `T → M<R>` (где M — та же монада), разворачивает и переоборачивает без двойной вложенности. В Scala это `flatMap`, в Haskell — `>>=`, в Kotlin — тоже `flatMap`. Практическое правило: если после `map` ты получил `Stream<Stream<T>>` или `Optional<Optional<T>>` — нужен `flatMap`.
...
**↳ Ответ:** `Spliterator` — итератор, который умеет делиться пополам (`trySplit()`). В parallel stream ForkJoinPool рекурсивно вызывает `trySplit()`, пока размер части не опустится ниже порога — тогда каждый кусок обрабатывается в отдельной задаче. Характеристики `Spliterator` влияют на эффективность: `SIZED` (знает размер → лучшее разбиение), `ORDERED` (нужно соблюдать порядок → дороже), `CONCURRENT` (источник потокобезопасен → без копирования). `ArrayList` имеет `SIZED + ORDERED` → хорошо делится; `LinkedList` — только `ORDERED` без SIZED → плохо, делится медленно.
...
**↳ Ответ:** Только через JMH (Java Microbenchmark Harness) — не через `System.currentTimeMillis()`, который даёт неточные результаты из-за JIT прогрева. JMH корректно прогревает JVM, выполняет несколько итераций и усредняет. Сравниваете два теста: один с `stream()`, другой с `parallelStream()` на реальном объёме данных и реальной операции. Правило: если на `N` элементов с вашей операцией параллельный вариант не ускоряет хотя бы в 1.5–2x — overhead съел выигрыш. Никогда не замеряйте на маленьких коллекциях — результат будет шумом.
...
**↳ Ответ:** Побеждает последний вызов в цепочке — он определяет режим всего pipeline целиком. Если цепочка заканчивается `.sequential()`, весь стрим будет последовательным, даже если в начале стоит `.parallelStream()`. Режим — это свойство всего pipeline, а не отдельных шагов. Частично-параллельных стримов не существует. Правило: `.parallel()` и `.sequential()` — переключатель, а не фильтр для конкретной операции.
...
**↳ Ответ:** Потому что коллектор не использует один общий `ArrayList`. В parallel stream каждый поток работает со своим локальным аккумулятором (своим `ArrayList`), созданным через `supplier()`. Параллельные потоки никогда не пишут в один список одновременно. Финально все частичные списки объединяются через `combiner()` — `addAll` — уже в одном потоке. Именно поэтому у `Collector` есть `combiner()`: он нужен для безопасного объединения результатов параллельной обработки.
...
**↳ Ответ:** `CompletableFuture.supplyAsync()` без явного `Executor` тоже использует `ForkJoinPool.commonPool()` — тот же пул, что и `parallelStream()`. Если ваш `parallelStream()` запустит долгие или блокирующие операции и займёт все потоки пула, `CompletableFuture.supplyAsync()` встанет в очередь. В Spring Boot это создаёт скрытый дедлок: HTTP-поток ждёт `CompletableFuture`, а тот ждёт освобождения `commonPool`, занятого `parallelStream()`. Правило: для production использовать кастомный `ForkJoinPool` для тяжёлых задач, не полагаться на `commonPool`.
...
**↳ Ответ:** Effectively final — переменная, которой не присваивается новое значение после инициализации (без ключевого слова `final`, но ведёт себя как final). Java требует этого, потому что лямбда захватывает значение переменной, а не ссылку на стек-фрейм. Стек-фрейм метода уничтожается после возврата, а лямбда может жить дольше. Если бы переменная могла меняться, лямбда и вызывающий код видели бы разные значения — неопределённое поведение. Правило: если компилятор ругается «variable used in lambda should be effectively final» — заведите новую переменную с копией значения.
...
**↳ Ответ:** `forEach` — единственная terminal-операция, явно спроектированная для side effects: отправить email, сохранить в БД, залогировать. Это оправдано и является идиоматичным использованием. Среди intermediate-операций — `peek` для `log.debug` в отладочных целях. Более сложный случай: если операция идемпотентна и атомарна (запись в `ConcurrentHashMap.put`), side effect в параллельном `forEach` может быть оправдан, но нужно явно документировать. Правило: side effect в terminal — нормально; side effect в intermediate — только `peek` для логов, никогда для бизнес-логики.
...
**↳ Ответ:** `groupingBy` в parallel stream создаёт несколько частичных `HashMap`, а затем объединяет их (merge) в одном потоке — это дополнительный шаг. `groupingByConcurrent` использует один общий `ConcurrentHashMap`, в который все потоки пишут напрямую без объединения — исключает финальный merge-шаг. Но есть цена: `groupingByConcurrent` помечен `CONCURRENT + UNORDERED`, то есть порядок элементов в группах не гарантирован. Правило: если порядок внутри групп не важен — `groupingByConcurrent` быстрее в parallel; если порядок нужен — `groupingBy`.
...
**↳ Ответ:** `reduce` по контракту должен создавать новый объект на каждом шаге, а не мутировать аккумулятор — потому что в parallel режиме identity передаётся во все потоки одновременно. Если вы мутируете identity (например, `ArrayList`) — все потоки пишут в один список без синхронизации, гонка данных. `collect` создаёт отдельный аккумулятор для каждого потока через `supplier()` — каждый поток работает со своим, потом combiner объединяет. В последовательном режиме это `O(n)` по памяти; `reduce` с конкатенацией списков — `O(n²)` из-за каждый раз создаваемой копии.
...
**↳ Ответ:** `reduce` правильнее, когда результат — скалярное иммутабельное значение: сумма, максимум, конкатенация строки через `StringBuilder.toString()`, произведение. Если функция ассоциативна и не требует мутабельного контейнера — `reduce` семантически точнее. Также `reduce(BinaryOperator)` возвращает `Optional<T>` — что правильно для пустого стрима, в отличие от `collect`, который всегда возвращает (пустую) коллекцию. Правило: скаляр и иммутабельная свёртка → `reduce`; коллекция или мутабельный контейнер → `collect`.
...
**↳ Ответ:** Зависит от версии Java. До Java 9 — вызывался. В Java 9+ компилятор оптимизирует `count()` и может полностью пропустить `peek` Consumer — если стрим SIZED и нет операций, меняющих размер, `count` возвращает размер напрямую из `Spliterator.estimateSize()` без прохода по элементам. Это задокументировано: `peek` с `count()` ненадёжен. Правило: никогда не завязывайте бизнес-логику на `peek` — это наглядная демонстрация того, что ленивость + оптимизация JVM могут «убрать» ваш Consumer.
...
**↳ Ответ:** Нельзя — `Stream` одноразовый и stateful, после terminal-операции брошен `IllegalStateException`. Для «ленивого поставщика» используйте `Supplier<Stream<T>>`: `private final Supplier<Stream<Order>> activeOrders = () -> orders.stream().filter(Order::isActive)`. Каждый вызов `activeOrders.get()` создаёт свежий стрим. Если источник данных меняется, каждый `get()` отражает актуальное состояние. Это чистый паттерн: стрим — одноразовый вид на данные, `Supplier` — фабрика этого вида.
...
**↳ Ответ:** Бесконечный стрим работает только потому, что intermediate-операции ленивы — элементы генерируются по одному, только по требованию terminal-операции. Без ленивости `filter` пришлось бы материализовать весь бесконечный стрим перед передачей результата — программа зависла бы. `Stream.iterate(0, n -> n + 1)` генерирует натуральные числа; `Stream.generate(Math::random)` — бесконечный поток случайных чисел. Остановка — через `limit(n)`, `findFirst()`, `takeWhile()` (Java 9+). Правило: бесконечный стрим без short-circuit terminal = бесконечный цикл.
...
**↳ Ответ:** Исключение в лямбде внутри стрима бросается в момент вызова terminal-операции, а не в момент построения pipeline. Если `map` содержит `throw new RuntimeException()`, это произойдёт при `collect()`, не при `.map()`. Это означает, что `try-catch` нужно оборачивать вокруг terminal-операции. Проблема с checked exceptions: лямбды в `map`/`filter` не могут бросать checked exceptions — компилятор запретит. Обходной путь: обернуть в unchecked (`UncheckedIOException`, кастомный wrapper), или использовать библиотеку вроде Vavr с `Try`.
...
**↳ Ответ:** «Barrier» означает: чтобы отсортировать элементы, нужно увидеть их ВСЕ — нельзя передать первый отсортированный элемент дальше, не зная остальных. Поэтому `sorted()` разрывает ленивость: до него — ленивый pipeline; после него — элементы уже в памяти в массиве. В parallel stream это ещё дороже: все потоки должны завершить свою часть, передать результаты в единый поток, который выполняет финальную сортировку — глобальный барьер синхронизации. Если `sorted()` стоит в parallel pipeline — параллелизм теряется именно на этом шаге.
...
**↳ Ответ:** `anyMatch` → `false` (нет ни одного элемента, удовлетворяющего условию); `allMatch` → `true` (нет ни одного нарушителя условия — vacuous truth); `noneMatch` → `true` (нет ни одного совпадения). Это математически корректно: `allMatch(P)` ≡ «не существует x, для которого ¬P(x)» — на пустом множестве нарушителей нет. Практическое правило-ловушка: `emptyStream.allMatch(x -> false)` возвращает `true` — если про это не знаешь, можно получить тихий баг в валидации.
...
**↳ Ответ:** В parallel stream как только один поток находит «решающий» элемент (`true` для `anyMatch`, `false` для `allMatch`), остальным потокам посылается сигнал отмены — они могут прекратить обработку. Но «минимальное число раз» — не гарантировано: ForkJoinPool разбил данные на части ещё до проверки, и часть потоков уже начала работу. Гарантировано только завершение без полного прохода по всем элементам при раннем нахождении. Практически: в parallel эти операции быстрее на больших данных, но у каждого потока минимум несколько вызовов предиката.
...
**↳ Ответ:** Encounter order — гарантированный порядок, в котором элементы проходят через стрим, совпадающий с порядком источника. Источники с encounter order: `List`, `array`, `LinkedList` (порядок вставки), `LinkedHashSet`, `LinkedHashMap`. Источники без: `HashSet`, `HashMap` — порядок итерации не определён. Важность: если источник UNORDERED, `findFirst()` семантически равен `findAny()`. `sorted()` устанавливает encounter order. В parallel stream UNORDERED-стримы параллелятся эффективнее — не нужно отслеживать «исходный» порядок.
...
**↳ Ответ:** `toMap` предполагает уникальность ключей — дубликат без merge-функции бросает `IllegalStateException`. `groupingBy` рассчитан на множественные элементы с одинаковым ключом — возвращает `Map<K, List<V>>`. Правило выбора: один объект → один ключ, ключи уникальны → `toMap`; несколько объектов → один ключ (классификация/группировка) → `groupingBy`. `toMap` гибче (можно задать и ключ, и значение независимо), `groupingBy` — специализирован для группировки с возможностью downstream-агрегаций.
...
**↳ Ответ:** Если результат — «список элементов по ключу» (`Map<K, List<V>>`) — всегда `groupingBy`, это его родная семантика. `toMap` с merge хорош для агрегации скаляров: суммирование, взятие первого, конкатенация. Но для нескольких downstream-агрегаций одновременно (и count, и сумма, и max) `groupingBy` с `Collectors.summarizingDouble` или `teeing` (Java 12+) гораздо чище. Правило: если merge-функция у вас это `(a, b) -> { a.addAll(b); return a; }` — вы неправильно используете `toMap`, возьмите `groupingBy`.
...
**↳ Ответ:** `orElse(value)` вычисляет значение **всегда**, независимо от того, есть ли значение в `Optional`. `orElseGet(supplier)` вычисляет значение **только если** Optional пустой. Если default-значение дорогое (например, `orElse(expensiveService.getDefault())`) — при наличии значения в Optional вы всё равно платите за `expensiveService.getDefault()`. Правило: `orElse` — для констант и уже созданных объектов; `orElseGet` — для всего, что требует вычисления, создания объекта или вызова метода.
