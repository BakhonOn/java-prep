...
**↳ Ответ:** `@Value` — аннотация процессора, генерирует код через APT; record — языковой примитив JVM. Record не требует зависимости Lombok, нативно работает с pattern matching в `switch`/`instanceof`, поддерживается всеми инструментами без плагинов, и интегрируется с sealed interfaces для алгебраических типов. Правило: проект Java 16+ — всегда record; Lombok `@Value` оставляйте только в legacy-коде или там, где нужен `@Builder` на неизменяемом объекте.
...
**↳ Ответ:** JPA требует: non-final класс (record неявно `final`), no-arg конструктор (record его не генерирует), изменяемые поля для lazy loading и dirty checking. Record нарушает все три требования. Для маппинга из БД использовать стандартные `@Entity` классы с Lombok, либо применять record как проекцию (Spring Data Projections, JDBC row mapper): запрос возвращает record напрямую, без хранения через JPA. Правило: Entity — изменяемые классы; Record — DTO/проекции на границе слоёв.
...
**↳ Ответ:** Sealed interface определяет закрытый набор реализаций, record реализует каждый вариант. Пример: `sealed interface Result<T> permits Success, Failure`. `record Success<T>(T value) implements Result<T>{}`, `record Failure<T>(String error) implements Result<T>{}`. В `switch` с pattern matching компилятор проверяет полноту — не пропущен ли вариант. Это аналог Scala/Kotlin `sealed class` — типобезопасные суммарные типы без `instanceof`-лапши.
...
**↳ Ответ:** Переопределяют аксессор для: (1) возврата defensive copy изменяемого поля (`public List<String> tags() { return List.copyOf(tags); }`); (2) скрытия чувствительных данных (аксессор `password()` возвращает `"****"`); (3) ленивого вычисления derived value (кэширование результата дорогой операции в `static Map`). Правило: аксессор должен быть чистой функцией — без побочных эффектов и с тем же логическим значением. Если нужна сложная логика — лучше отдельный метод с другим именем.
...
**↳ Ответ:** С Jackson 2.12+ и Java 16+ record поддерживается нативно без конфигурации: сериализация использует аксессоры `name()`, десериализация — канонический конструктор через `ConstructorPropertiesAnnotationIntrospector`. Spring Boot 2.5+ настраивает это автоматически. Если Jackson < 2.12 или нужна точная настройка — добавить `@JsonProperty` на компоненты record или подключить `jackson-modules-java8`. Правило: в Spring Boot 2.5+ record работает как DTO «из коробки», никакой конфигурации не нужно.
...
**↳ Ответ:** Компактный конструктор короче и читаемее: не нужно перечислять параметры в заголовке и писать `this.x = x` — компилятор делает это сам. Ключевое отличие: в компактном конструкторе параметры ещё мутабельны (можно переприсвоить для нормализации), тогда как в каноническом нужно использовать `this.field = normalizedValue`. Правило: используйте компактный конструктор для валидации и нормализации (trim, toLowerCase, defensive copy); канонический — когда нужны разные значения для поля и параметра конструктора.
...
**↳ Ответ:** В компактном конструкторе переприсвоить параметр через `List.copyOf(list)` — это создаёт новый неизменяемый список (`UnsupportedOperationException` при add/remove). Плюс при возврате из аксессора добавить defensive copy если нужно (`public List<String> tags() { return List.copyOf(tags); }`). `List.copyOf` также защищает от null-элементов (бросает NPE). Правило: иммутабельный record = иммутабельные компоненты; если компонент изменяемый — defensive copy в конструкторе.
...
**↳ Ответ:** Телескопический конструктор — антипаттерн из нескольких конструкторов с разным числом параметров (сложно читать, порядок параметров неочевиден). В record дополнительные конструкторы должны вызывать канонический, что неудобно. Вместо этого: `record Money(long cents, String currency)` + `static Money ofDollars(double d)`, `static Money ofEuros(double e)` — именованные фабрики с понятным намерением. Правило: если нужно несколько способов создания — статические фабрики с осмысленными именами лучше перегруженных конструкторов.
...
**↳ Ответ:** `Collections.unmodifiableList(original)` — тонкая обёртка вокруг оригинального списка. Если изменить `original` после создания обёртки — изменения видны через обёртку (view, не копия). `List.copyOf(original)` создаёт полностью независимую копию — изменения `original` не влияют. Для record правило однозначно: `List.copyOf()` — потому что нужна независимость от переданного списка. Также `List.copyOf` запрещает null-элементы, что добавляет дополнительную валидацию.
...
**↳ Ответ:** `hashCode` record вычисляется каждый раз при вызове (не кэшируется) на основе всех компонентов. Если `List` компонент изменился после помещения в Map — следующий вызов `hashCode` вернёт другое значение. HashMap ищет ключ по bucket = `hashCode % capacity` — с новым hashCode поиск пойдёт в другой bucket и не найдёт запись. Правило: ключи HashMap должны быть неизменяемыми — используйте только иммутабельные компоненты (String, Long, LocalDate, другие record с иммутабельными полями).
...
**↳ Ответ:** Type erasure стирает параметры типа при компиляции — в байткоде остаётся только raw type. Выбрали этот подход для обратной совместимости: код, скомпилированный до Java 5 (работающий с raw `List`), должен был продолжать работать с новым кодом `List<String>` без перекомпиляции. Сохранение типов (reified generics, как в C# и будущей Project Valhalla) потребовало бы несовместимого изменения JVM. Цена erasure: нельзя `instanceof T`, `new T()`, `T.class` в рантайме.
...
**↳ Ответ:** Нельзя в рантайме: `instanceof List<String>` (только `instanceof List<?>`), `new T()` — тип T неизвестен JVM, `new T[]` — нельзя создать типизированный массив, `List<String>.class` — только `List.class`, перехватить generic-исключение `catch (E ex)`. Всё это компилятор запрещает. Обходы: передавать `Class<T>` явно, использовать `TypeToken`/`ParameterizedTypeReference` (Guava, Spring) для сохранения информации о типе через anonymous subclass.
...
**↳ Ответ:** Три способа: (1) `Class<T> clazz` — передать класс явно, вызвать `clazz.getDeclaredConstructor().newInstance()` через рефлексию; (2) `Supplier<T> constructor` — предпочтительно, без рефлексии: `repository = new Repository<>(User::new)`; (3) абстрактный фабричный метод — подкласс переопределяет `protected T createDefault()`. Правило: `Supplier<T>` — чистый и безопасный вариант; `Class<T>` — только если нужна рефлексия для других целей (например, Spring `@Bean`).
...
**↳ Ответ:** Массивы ковариантны для совместимости с кодом до дженериков (например, `Arrays.sort(Object[] a)`). Цена — проверка типа при записи в рантайме (`ArrayStoreException`): `Object[] arr = new String[3]; arr[0] = 42; // исключение в рантайме`. Дженерики инвариантны для безопасности на этапе компиляции: нельзя `List<Object> = new ArrayList<String>()` — ошибка компиляции. Правило: дженерики дают ошибку раньше (компиляция); массивы — позже (рантайм). Предпочитайте `List<T>` массивам в дженерик-коде.
...
**↳ Ответ:** T заменяется на первый bound: `<T extends Comparable<T>>` → T становится `Comparable` в байткоде. `<T extends Number & Comparable<T>>` → T становится `Number` (первый bound — класс). Вызовы методов Comparable добавляются как checkcast при необходимости. Практическое следствие: `getClass()` на T вернёт runtime-тип объекта, а не T — стирание не влияет на фактический класс экземпляра.
...
**↳ Ответ:** `List<? extends Number>` может быть `List<Integer>`, `List<Double>`, `List<BigDecimal>` — компилятор не знает конкретный тип. Если разрешить `list.add(1.5)` — можно случайно добавить `Double` в `List<Integer>`, нарушив типобезопасность. Компилятор защищает от этого, запрещая любую запись (кроме `null`). Правило: `? extends` = только читать; знание «там Number-совместимые» недостаточно — нужно знать точный тип для записи.
...
**↳ Ответ:** Метод `sum(List<Number>)` не примет `List<Integer>` — инвариантность. Метод `sum(List<? extends Number>)` принимает `List<Integer>`, `List<Double>`, `List<BigDecimal>` — максимальная гибкость для producer. Безопасность обеспечивается запретом записи: нельзя случайно изменить содержимое списка клиента. Правило (PECS): входящий параметр, из которого только читают — `? extends`; в который только пишут — `? super`. Возвращаемые типы — без wildcard.
...
**↳ Ответ:** Project Valhalla — долгосрочный проект JVM, вводящий value types (value classes) и universal generics. Цель: `List<int>`, `List<double>` — дженерики над примитивами без boxing. Value types — объекты без идентичности (сравниваются по значению), могут хранится на стеке и в массивах без индирекции. Это устранит главный источник GC pressure при работе с коллекциями чисел. На 2024 год — Preview features появляются в Java 22+, но полная стабилизация ещё впереди.
...
**↳ Ответ:** `List<?>` — типобезопасный wildcard: компилятор запрещает добавлять элементы (кроме `null`), чтение возвращает `Object`, но предупреждений нет. Raw `List` — полное отключение generics: добавить можно что угодно, компилятор выдаёт unchecked warning, в рантайме возможен `ClassCastException`. Правило: `List<?>` используйте когда нужно принять коллекцию любого типа только для чтения; raw type допустим лишь при работе с legacy API до Java 5.
...
**↳ Ответ:** `Supplier<T>` — обычный вызов метода, проверяемый компилятором, без рефлексии и checked exceptions. `Class<T>` требует `getDeclaredConstructor().newInstance()` — рефлексия, `try/catch` для `InvocationTargetException`/`InstantiationException`, и конфликты с модульной системой. Плюс `Supplier` может быть любой фабрикой, а не только no-arg конструктором: `() -> new User("default")`. Правило: всегда предпочитайте `Supplier<T>` — чище, безопаснее, тестируемее.
...
**↳ Ответ:** Ковариантность массивов — историческое решение для совместимости с кодом до Java 5: методы вроде `Arrays.sort(Object[])` должны были принимать `String[]`. Цена — проверка типа при записи делается в рантайме (`ArrayStoreException`). Дженерики же проверяются статически — ошибка на этапе компиляции. Правило: ковариантность массивов — legacy-компромисс; в новом коде предпочитайте `List<T>`, где безопасность гарантируется компилятором, а не рантаймом.
...
**↳ Ответ:** При `class StringBox extends Box<String>` переопределении `set(String value)` — erasure превращает его в `set(Object value)`. Чтобы полиморфизм работал через ссылку типа `Box<?>`, компилятор генерирует bridge method `set(Object v) { set((String) v); }`. Без него вызов через стёртую ссылку не нашёл бы переопределённый метод. Правило: bridge methods невидимы в коде, но видны через рефлексию (`method.isBridge() == true`) — всегда фильтруйте их при переборе методов через `getDeclaredMethods()`.
...
**↳ Ответ:** CRTP — паттерн где класс параметризован самим собой: `abstract class Builder<T extends Builder<T>>`. Это позволяет fluent-методам в базовом классе возвращать тип подкласса (`return (T) this`), а не базовый — цепочки вызовов не теряют тип. Применение: `new UserBuilder().name("x").age(25)` возвращает `UserBuilder`, а не `Builder`. Правило: используйте CRTP для fluent builders в иерархиях — без него `name()` в базовом классе возвращал бы `Builder`, и вызвать метод подкласса после него было бы невозможно без каста.
...
**↳ Ответ:** `List<? extends T>` — ковариантен: принимает `List<T>` и `List<SubT>`, только чтение (запись запрещена — компилятор не знает точный тип). `List<? super T>` — контравариантен: принимает `List<T>` и `List<SuperT>`, запись разрешена, чтение возвращает `Object`. Правило PECS: **P**roducer **E**xtends, **C**onsumer **S**uper — если коллекция является источником данных, используйте `? extends`; если приёмником — `? super`.
...
**↳ Ответ:** `Class.getDeclaredMethods()` возвращает и bridge methods, помеченные `isBridge() && isSynthetic()`. Spring фильтрует их через `ReflectionUtils.USER_DECLARED_METHODS`: если не фильтровать, `@Transactional`/`@Cacheable` могут применяться к bridge method, а не к реальному — AOP-перехват сработает только при вызове через стёртую ссылку, но не через конкретный тип. Правило: при переборе методов через рефлексию всегда проверяйте `!method.isBridge()` — иначе получите дублирование аннотаций и непредсказуемое поведение прокси.
...
**↳ Ответ:** T заменяется на первый bound — класс, если он есть: `<T extends Number & Comparable<T>>` → T = `Number`. Где нужны методы `Comparable`, компилятор добавляет `checkcast Comparable`. Правило: класс всегда должен быть первым в списке bounds (`<T extends Number & Serializable>`); интерфейсы могут идти в любом порядке после него — первый bound и есть «стёртый» тип T в байткоде.
...
**↳ Ответ:** JVM гарантирует создание каждого enum-значения ровно один раз при загрузке класса. При сериализации enum не проходит через `readObject()` — JVM восстанавливает экземпляр через `Enum.valueOf()` по имени, всегда возвращая тот же объект. Рефлексия не может создать второй экземпляр: `Constructor.newInstance()` бросает `IllegalArgumentException`. Правило: enum Singleton защищён от всех атак (сериализация, рефлексия, клонирование) на уровне JVM без единой дополнительной строки кода — именно поэтому Effective Java рекомендует его как единственно правильный вариант.
