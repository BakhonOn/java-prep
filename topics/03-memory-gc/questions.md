...
**↳ Ответ:** TLAB — каждому потоку выделяется свой личный буфер в Eden. Аллокация внутри TLAB — просто сдвиг указателя (pointer bump), без синхронизации с другими потоками. Когда TLAB заполнен — поток запрашивает новый (с CAS), а не блокирует общий Eden при каждом `new`. Правило: если объект не помещается в TLAB, JVM аллоцирует его напрямую в Eden с блокировкой — избегай частых крупных объектов в горячих путях.
...
**↳ Ответ:** Promotion failure — в Old Gen нет места для объектов, переживших Minor GC → JVM делает Full GC (дорого). Concurrent mode failure (CMS-термин) / аналог в G1 — concurrent marking не успел завершиться до того, как Old Gen переполнился → JVM падает в Serial Full GC. Оба сценария означают STW в несколько секунд. Профилактика: следить за `InitiatingHeapOccupancyPercent` (G1 начинает marking при этом % заполнения Old Gen; снизь до 35–40 если часто бывает concurrent mode failure).
...
**↳ Ответ:** Утечка в Metaspace — classloader не выгружается, потому что где-то осталась сильная ссылка на него или на загруженный им класс (например, ThreadLocal, статическое поле сторонней библиотеки, JDBC driver в BootClassLoader). Признак: Metaspace растёт после каждого redeploy. Диагностика: в MAT → "List classloaders" → найди дубли classloader'ов. В JFR: `ClassLoaderStatistics` event. Правило: если после N redeploy'ев в Metaspace N копий classloader'а — утечка через статику или ThreadLocal.
...
**↳ Ответ:** Concurrent = GC-поток работает одновременно с потоками приложения. В G1: STW фазы — Initial Mark (короткая, ≈Minor GC), Remark (короткая), Cleanup (частично). Concurrent фазы — Concurrent Mark (обход графа живых объектов), Concurrent Cleanup (освобождение полностью пустых регионов). Evacuation (Minor GC и Mixed GC) — всегда STW. Правило: concurrent — значит без полной остановки, но не значит ноль пауз.
...
**↳ Ответ:** Humongous объекты (> 50% размера региона G1) аллоцируются напрямую в Old Gen, минуя Young. Это плохо по двум причинам: 1) они не собираются Minor GC — только при Mixed или Full GC; 2) они занимают целые регионы с фрагментацией (объект на 2.5 региона занимает 3). Если приложение часто создаёт большие массивы (буферы, сериализация) — увеличь `G1HeapRegionSize` чтобы порог humongous стал выше, или избегай крупных временных аллокаций.
...
**↳ Ответ:** Colored pointers — ZGC хранит метаданные (marked, remapped, finalizable) прямо в битах 64-битного указателя (биты 42–45). Load barrier — код, вставляемый JIT при каждом чтении ссылки из heap: проверяет цвет и при необходимости обновляет указатель на новый адрес перемещённого объекта. Это позволяет relocate объекты конкурентно: старые ссылки «лечатся» при чтении, а не при перемещении. Цена — overhead на каждое чтение ссылки (~1–3% throughput).
...
**↳ Ответ:** Brooks pointers — дополнительное слово в каждом объекте (увеличивает его размер), указывающее на актуальный адрес. При доступе к перемещённому объекту store/load barrier делает лишнее чтение через Brooks pointer. ZGC хранит метаданные в битах самого указателя — не увеличивает объект, но требует маскирования адреса при каждом обращении. Brooks overhead ≈ +1 слово на объект (память); ZGC overhead ≈ маскирование адреса (CPU). В итоге ZGC обычно эффективнее по памяти, Shenandoah иногда выигрывает при малом heap.
...
**↳ Ответ:** Safepoint — точка в байткоде, где поток гарантированно может быть остановлен JVM (между байткод-инструкциями, при вызовах методов). Time-to-safepoint — время от запроса GC до момента когда все потоки дошли до safepoint. Длинный цикл без safepoint-опросников (например, tight loop по массиву с JIT) удерживает поток: GC ждёт его, и STW-пауза начинается позже. Правило: `-XX:+PrintSafepointStatistics` покажет проблемных "задержантов"; JIT вставляет опросники в обратные ветки циклов, но иногда пропускает.
...
**↳ Ответ:** JFR (Java Flight Recorder) с overhead < 1–2% — предпочтительный метод: `jcmd <pid> JFR.start duration=60s settings=profile filename=/tmp/gc.jfr`, затем открыть в JMC → GC tab → смотреть Pause Duration, GC Cause, Allocation Rate. Альтернатива — GC логи (`-Xlog:gc*`) + утилита GCViewer/gceasy.io для визуализации. В Prometheus/Grafana — JVM Micrometer метрики `jvm_gc_pause_seconds` дают p50/p99 пауз в реальном времени. Смотрю: p99 pause, частоту Full GC, allocation rate vs promotion rate.
...
**↳ Ответ:** До Java 10 JVM читала `/proc/meminfo` хоста и не знала о cgroup-лимитах контейнера. С Java 10+ `UseContainerSupport=true` по умолчанию — JVM читает cgroup memory limits и корректно считает доступную память. Явно включать не нужно в Java 10+, но в Java 8u191+ нужен флаг. Без него JVM в контейнере на 512 МБ может попытаться взять 25% RAM хоста (например, 4 ГБ) — OOMKiller убьёт процесс без `OutOfMemoryError` в логах.
...
**↳ Ответ:** Заранее настрой: `-XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=/persistent-volume/` (не tmpfs, переживёт restart), GC логи с ротацией, JFR с continuous recording (`dumponexit=true`). Если dump не был настроен — анализируй: GC логи (последние паузы, allocation rate до OOM), метрики Prometheus (тренд Old Gen за часы до падения), логи приложения (какие операции выполнялись). Если воспроизводимо — запусти с уменьшенным heap `-Xmx` для ускорения воспроизведения и сними dump.
...
**↳ Ответ:** На уровне ОС: `ulimit -u` (max user processes) и `cat /proc/sys/kernel/threads-max`. На уровне JVM: `jstack <pid> | grep "java.lang.Thread.State" | wc -l` — реальное число потоков, сравни с ожидаемым. Также: нативная память под стеки = threads × Xss; при 1000 потоков × 1МБ = 1ГБ нативной памяти вне heap. В контейнере: лимит памяти включает и нативную память JVM, не только heap. Правило: runaway thread pool или ThreadLocal утечки — основные причины.
...
**↳ Ответ:** Dominator Tree — дерево, где объект A является "доминатором" объекта B если все пути от GC roots до B проходят через A. Retained Heap объекта A = всё что освободится если удалить A. MAT строит Dominator Tree чтобы найти "держателей" памяти: один Map на 2 ГБ retained heap сразу видно. Алгоритм: смотри на узлы с огромным retained size → раскрывай → найди retention path до GC root через "Path to GC Roots".
...
**↳ Ответ:** Запуск с continuous recording: `-XX:StartFlightRecording=name=prod,settings=default,maxage=1h,maxsize=500m,dumponexit=true,filename=/logs/app.jfr`. Overhead < 1% при `settings=default`. Для поиска утечки ключевые события: `jdk.OldObjectSample` (живые объекты в Old Gen — покажет allocation stack trace самых старых объектов), `jdk.GCHeapSummary` (тренд heap), `jdk.ClassLoaderStatistics`. В JMC: вкладка Memory → Old Objects → найди доминирующий класс с retention path.
...
**↳ Ответ:** MAT умеет работать с dump больше RAM за счёт memory-mapped файлов (mmap) — открывает частично. В `MemoryAnalyzer.ini` увеличь `-Xmx` MAT до максимума доступной RAM (например, `-Xmx7g`). MAT сжимает индекс dump на диске — сам индекс ~10–20% от размера dump. Если этого мало: используй `ParseHeapDump.sh` в batch-режиме для генерации отчётов без GUI, или переедь на машину с достаточной RAM (облако). Альтернатива — `jhat` или VisualVM с ограниченным анализом.
...
**↳ Ответ:** Первый шаг — собрать косвенные данные: GC логи (если были настроены на persistent volume), Prometheus метрики heap до падения, логи приложения перед OOM. Воспроизведение: запусти под нагрузкой (Gatling/JMeter) с уменьшенным heap (`-Xmx`) и включёнными `-XX:+HeapDumpOnOutOfMemoryError`, JFR. Если нагрузка непредсказуема — настрой canary pod с профайлером и жди повторения. После инцидента — обязательно добавь dump-флаги на persistent volume; без этого следующий OOM снова "без улик".
...
**↳ Ответ:** Каждый Thread хранит `ThreadLocalMap` — это поле самого объекта Thread. Thread является GC root (активный поток). Цепочка: Thread (GC root) → Thread.threadLocals (ThreadLocalMap) → Entry → value (твой объект). Пока поток живёт — значение достижимо и не собирается GC. В пуле потоков (Tomcat) поток никогда не умирает → все ThreadLocal-значения без `remove()` живут вечно. Ключи (ThreadLocal-объекты) — WeakReference, поэтому ключ может быть собран, но value остаётся (zombie entry).
...
**↳ Ответ:** WeakHashMap использует weak keys (ключи — WeakReference): когда ключ-объект не имеет strong ссылок снаружи — entry автоматически удаляется. HashMap с WeakReference в values — слабые значения: значение может собраться GC, но сам Entry в map останется (с null-value). WeakHashMap не защищает если: 1) value содержит strong ссылку на key — key никогда не соберётся; 2) key — интернированная строка или примитивная обёртка (кэшируется JVM и живёт вечно); 3) ты ждёшь немедленного удаления — очистка происходит при следующем обращении к map.
...
**↳ Ответ:** `-XX:+DisableExplicitGC` делает `System.gc()` полным no-op. Нужен когда: RMI или сторонняя библиотека вызывает `System.gc()` периодически (RMI по умолчанию каждую минуту) и это вызывает нежелательные Full GC. С NIO direct buffers ситуация сложная: Cleaner (финализатор direct buffers) запускается при GC — если `System.gc()` отключён, а direct buffers создаются быстро, `MaxDirectMemorySize` может переполниться. Тогда нужно явно вызывать `cleaner.clean()` или использовать `-XX:+ExplicitGCInvokesConcurrent` вместо полного отключения.
...
**↳ Ответ:** Правильный путь — явный вызов Cleaner: `((sun.nio.ch.DirectBuffer) buf).cleaner().clean()` (Java 8) или через `MemorySegment` (Java 22+). `System.gc()` "помогает" потому что GC запускает Cleaner для финализируемых объектов — но это недетерминировано, может не запуститься или запуститься слишком поздно (OOM до следующего GC). Плохая практика потому что: 1) может вызвать Full GC с паузой; 2) нет гарантии освобождения именно нужных буферов. Лучший паттерн — try-with-resources wrapper вокруг direct buffer с явным clean() в close().
