...
**↳ Ответ:** Примитивное поле класса (`int age` в `User`) хранится в heap — как часть объекта `User`. Стек содержит только локальные примитивы и ссылки внутри методов. Поле объекта внутри другого объекта — тоже heap: примитив лежит инлайн внутри объекта-владельца, ссылочный тип — ссылка инлайн, но объект, на который она указывает, отдельно в heap. Правило: всё что принадлежит объекту — в heap вместе с ним.
...
**↳ Ответ:** Литерал `"hello"` помещается в String Pool (с Java 7 — в heap). Одинаковые литералы — одна ссылка. `new String("hello")` — всегда создаёт новый объект в heap вне пула. Поэтому `"hello" == new String("hello")` — false (разные объекты в памяти), `"hello".equals(new String("hello"))` — true. Правило: для строк всегда используй equals, никогда ==.
...
**↳ Ответ:** Escape Analysis — анализ JIT, определяющий «убегает» ли объект за пределы метода (возвращается, передаётся в другой поток, сохраняется в поле). Если объект не убегает — JIT может: разместить его на стеке (Stack Allocation) или устранить полностью (Scalar Replacement — заменить объект на набор локальных переменных). На практике это убирает heap-аллокацию мелких DTO в тесных циклах. Включено по умолчанию; посмотреть через `-XX:+PrintEliminateAllocations`.
...
**↳ Ответ:** Marking (маркировка живых от roots) — в G1/ZGC частично concurrent, но initial mark и remark требуют short STW. Sweep (зачистка мёртвых) — в конкурентных GC concurrent. Compact (перемещение живых для устранения фрагментации) — в Serial/Parallel всегда STW; в ZGC/Shenandoah concurrent. Правило: compact — самая дорогая фаза; именно конкурентная компактизация делает ZGC/Shenandoah лидерами по latency.
...
**↳ Ответ:** Да, GC соберёт оба — JVM использует tracing GC (обход от roots), а не reference counting. Циклические ссылки для него не проблема: если цикл не достижим от roots — весь граф считается мусором. Reference counting не используется именно потому что не справляется с циклами: счётчики A и B не обнулятся при взаимных ссылках. Кроме того, каждое присвоение требовало бы атомарного декремента/инкремента — дорого.
...
**↳ Ответ:** Признак утечки — Old Gen продолжает расти после каждого Full GC и не возвращается к базовому уровню. Высокое потребление — стабильно. В heap dump смотрю: 1) Dominator Tree — кто держит больше всего retained heap. 2) Retained vs Shallow size: огромный retained при маленьком shallow — держит большой граф. 3) Leak Suspects Report в MAT. 4) Compare двух dump с интервалом — нарастающий класс и есть утечка.
...
**↳ Ответ:** WeakReference позволяет GC собрать объект когда нет strong ссылок — кэш не удерживает объект принудительно. WeakHashMap использует weak keys: запись автоматически удаляется когда ключ собирается GC. Применяю: кэш вычисленных метаданных, привязанных к объектам с непредсказуемым lifecycle (`WeakHashMap<Widget, RenderedData>`). Ловушка: WeakHashMap бесполезен если value содержит strong ссылку на ключ — ключ никогда не соберётся.
...
**↳ Ответ:** Tenuring threshold — возраст объекта (число Minor GC, пережитых до промоции в Old Gen), дефолт 15. Promotion failure — когда в Old Gen нет места для выживших из Young → JVM вынуждена сделать Full GC. Для REST-сервиса: большой Young Gen (увеличь `-XX:NewRatio` до 1, т.е. равные части) уменьшает промоции короткоживущих DTO. Если промоций много — смотри `-XX:+PrintTenuringDistribution` чтобы понять реальный возраст объектов.
