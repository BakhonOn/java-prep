...
**↳ Ответ:** JOIN FETCH гарантирует один SQL-запрос, но несовместим с пагинацией для коллекций и ломается при нескольких коллекциях (MultipleBagFetchException). @BatchSize сокращает N запросов до ceil(N/size) запросов и спокойно работает с пагинацией — Hibernate просто группирует IN-запросы. Правило выбора: JOIN FETCH — когда нужен гарантированный минимум запросов без пагинации; @BatchSize — когда есть пагинация или несколько коллекций одновременно.
...
**↳ Ответ:** Если обе коллекции объявлены как `List` — Hibernate выбросит `MultipleBagFetchException` при старте («cannot simultaneously fetch multiple bags»). Если хотя бы одна — `Set`, ошибки нет, но появляется декартово произведение: 10 заказов × 5 позиций = 50 строк в SELECT. Вывод-эвристика: два EAGER List — всегда ошибка; два EAGER Set — работает, но медленно и незаметно. Держите всё LAZY и подгружайте явно.
...
**↳ Ответ:** Три инструмента: (1) `@Query("SELECT o FROM Order o JOIN FETCH o.user WHERE ...")` — явный JOIN FETCH в JPQL для конкретного метода; (2) `@EntityGraph(attributePaths = {"user", "items"})` — декларативный граф загрузки над методом репозитория; (3) именованный `@NamedEntityGraph` на сущности + ссылка из репозитория. Эвристика: JOIN FETCH — для простых случаев в одном запросе; @EntityGraph — когда граф переиспользуется или нужна читаемость.
...
**↳ Ответ:** OSIV держит соединение с БД открытым на всё время HTTP-запроса, включая сериализацию JSON в контроллере — это значит, соединение занято даже в ожидании медленного клиента. При нагрузке 100 RPS × 200мс на ответ = 20 соединений постоянно в «холостом» ожидании, что исчерпывает connection pool. Дополнительно: N+1 запросы спрятаны в слое сериализации — их невозможно отследить без анализа SQL-логов. Отключите OSIV (`spring.jpa.open-in-view=false`) и явно управляйте загрузкой через JOIN FETCH + DTO.
...
**↳ Ответ:** SUBSELECT загружает коллекции всех уже загруженных родителей одним дополнительным запросом с подзапросом (`WHERE parent_id IN (SELECT ...)`). Это предпочтительнее JOIN FETCH когда: используется пагинация (JOIN FETCH с коллекцией ломает offset/limit), или когда коллекция нужна не сразу, а после какой-то фильтрации. Эвристика: JOIN FETCH — «загрузить всё сразу одним запросом»; SUBSELECT — «загрузить коллекции вторым запросом, но сразу для всех уже найденных родителей».
...
**↳ Ответ:** Через свойство `spring.jpa.properties.hibernate.default_batch_fetch_size=25` в `application.properties` — Hibernate применит BatchSize ко всем ленивым коллекциям и прокси-сущностям автоматически. Это самый быстрый способ снизить N+1 на уровне всего приложения без изменения кода. Однако глобальный BatchSize — компромисс: он применится везде, включая случаи, где JOIN FETCH был бы оптимальнее. Используйте как базовую защиту, а критические запросы оптимизируйте явно.
...
**↳ Ответ:** Вызов `persist()` на Removed-объекте (до flush) отменяет удаление — объект возвращается в состояние Persistent. Это специфическое поведение JPA: persist «спасает» Removed-сущность, если DELETE ещё не ушёл в БД. После flush объект снова под управлением контекста, DELETE не выполняется. Правило: до flush состояние Removed «мягкое» и обратимо через persist; после flush — необратимо (объект в БД удалён).
...
**↳ Ответ:** Три подхода: (1) Использовать `@Transactional(readOnly = true)` — Hibernate пропускает dirty checking при flush, случайные изменения не сохранятся; (2) Явно отсоединить объект через `entityManager.detach(entity)` перед локальными изменениями; (3) Никогда не изменять поля Entity для «вывода» — маппить в DTO внутри транзакции и работать с DTO. Эвристика: если метод только читает данные — всегда `readOnly = true`; если нужно изменить Entity временно — detach или DTO.
...
**↳ Ответ:** При FlushMode.AUTO (умолчание) Hibernate перед выполнением JPQL-запроса автоматически делает flush, если запрос затрагивает те же таблицы, что и несохранённые изменения. Таким образом, JPQL-запрос увидит актуальные данные. Но при FlushMode.COMMIT — flush не происходит, JPQL читает из БД «старые» данные, а объект в L1 уже «новый» — несоответствие. Ловушка: объект из L1 вернётся по id «новым» (из кэша), но запрос по WHERE вернёт «старые» строки.
...
**↳ Ответ:** Четыре стратегии: READ_ONLY (только чтение, самая быстрая, неизменяемые данные), READ_WRITE (мягкая блокировка при обновлении — гарантирует согласованность при concurrent writes), NONSTRICT_READ_WRITE (нет блокировок, возможно кратковременное устаревание — выше производительность, ниже согласованность), TRANSACTIONAL (поддержка полноценных транзакций с XA). READ_WRITE vs NONSTRICT_READ_WRITE: READ_WRITE временно «мягко блокирует» регион в кэше при изменении, предотвращая dirty read; NONSTRICT_READ_WRITE просто инвалидирует кэш после изменения — в момент инвалидации другой поток может прочитать устаревшее значение.
...
**↳ Ответ:** Ничего — L2-кэш живёт на уровне SessionFactory и не зависит от жизненного цикла отдельных сессий. `entityManager.clear()` очищает только L1 (PersistenceContext текущей сессии); закрытие сессии тоже не трогает L2. Это ключевое отличие: L1 умирает вместе с сессией, L2 переживает тысячи сессий. Очистить L2 можно только явно: `entityManager.getEntityManagerFactory().getCache().evictAll()`.
...
**↳ Ответ:** Hibernate при `readOnly = true` пропускает создание «снимков» (snapshots) загруженных объектов и не выполняет dirty checking при flush — это экономит память и CPU пропорционально количеству загруженных объектов. В Spring `readOnly = true` также сигнализирует драйверу JDBC и БД об оптимизациях (некоторые БД переключают сессию в read-only mode). Эвристика: все методы репозитория и сервиса, которые только читают данные, должны быть `readOnly = true` — это бесплатная оптимизация.
...
**↳ Ответ:** При FlushMode.COMMIT flush не происходит перед JPQL-запросами — запрос видит «старые» данные в БД, хотя в PersistenceContext уже есть «новые» изменения. Классическая ловушка: создали объект через persist, не flush, потом JPQL `SELECT COUNT(*)` — счётчик не учитывает только что добавленный объект. FlushMode.COMMIT оправдан только в batch-операциях, где JPQL-запросы внутри транзакции заведомо не конфликтуют с несохранёнными изменениями.
...
**↳ Ответ:** Hibernate ищет объект сначала в L1-кэше (не находит), затем в БД (SELECT по id — тоже не находит), и в этом случае создаёт новую Persistent-сущность с тем же id и копирует в неё состояние переданного объекта — поведение аналогично persist. При flush выполнится INSERT. Это неочевидно: merge с несуществующим id ведёт себя как «upsert» — вставка. Правило: Spring Data `save()` использует именно этот механизм — если id не null, но записи нет → INSERT, а не UPDATE.
...
**↳ Ответ:** Без `CascadeType.REFRESH` вызов `entityManager.refresh(parent)` перезагружает только родителя — дочерние сущности остаются с прежними данными. Если дочерние тоже могли измениться (например, нативный SQL изменил и parent, и children) — нужен `CascadeType.REFRESH` на связи, тогда refresh каскадируется на всю коллекцию. Эвристика: `CascadeType.REFRESH` добавляйте осознанно — для агрегатов, где дочерние могут изменяться независимо от JPA-слоя (триггеры, нативные запросы).
...
**↳ Ответ:** `StatelessSession` — облегчённая сессия без PersistenceContext (нет L1-кэша), без dirty checking и без каскадирования. Каждая операция (insert, update, delete) выполняется немедленно в SQL, объекты никогда не становятся Persistent. Используется исключительно для batch-обработки: импорт миллионов строк, ETL-операции, массовые обновления — там, где обычная Session накапливала бы объекты в L1 и вызывала OutOfMemoryError. Правило: обычные бизнес-операции — `Session`; bulk-данные без управления жизненным циклом — `StatelessSession`.
...
**↳ Ответ:** При `OptimisticLockException` текущая транзакция помечается как rollback-only — её нельзя продолжить. Retry должен запустить новую `@Transactional`-транзакцию, которая загрузит актуальную версию объекта из БД. В Spring: `@Retryable(value = ObjectOptimisticLockingFailureException.class, maxAttempts = 3)` на методе сервиса — но метод должен иметь `@Transactional` с `REQUIRES_NEW` или вызывающий код создаёт новую транзакцию. Ловушка: retry внутри той же транзакции бессмыслен — она уже помечена rollback-only и откатится при завершении.
...
**↳ Ответ:** Ключевой вопрос — вероятность конфликта и стоимость его обнаружения. Оптимистичная: конфликты редки, транзакция короткая, допустим retry при конфликте — нет блокировок в БД, высокий throughput. Пессимистичная: конфликты часты (например, несколько операторов резервируют один билет), транзакция длинная, повтор дорог или невозможен — гарантия «один победитель», но меньший параллелизм и риск дедлока. Эвристика: интернет-магазин с тысячами пользователей → оптимистичная; бронирование мест с десятками операторов → пессимистичная.
...
**↳ Ответ:** Две проблемы: (1) Точность часов — если сервер обновил запись дважды за один миллисекунд (или timestamp с секундной точностью), версии будут одинаковыми, конфликт не обнаружится — потерянное обновление без исключения; (2) Clock skew — в кластерных средах разные узлы имеют разное системное время, что нарушает монотонность версий. Числовой тип (Long/int) не зависит от часов и гарантированно монотонно возрастает. Правило: всегда используйте числовой `@Version`, Timestamp — только если нужна информация о времени последнего изменения отдельно от версии.
...
**↳ Ответ:** `CascadeType.REMOVE` срабатывает при `entityManager.remove(parent)` — удалении самого родителя, каскадируя DELETE на детей. `orphanRemoval = true` срабатывает при удалении элемента из коллекции (`order.getItems().remove(item)`) — даже если родитель не удаляется. Нужны оба вместе когда: дочерние объекты не должны существовать ни без родителя (`REMOVE`), ни без позиции в его коллекции (`orphanRemoval`). Стандартная комбинация для агрегатов: `cascade = CascadeType.ALL, orphanRemoval = true`.
...
**↳ Ответ:** Если тег уже существует в БД (Detached-объект с id), и вы добавите его в коллекцию поста с `CascadeType.PERSIST`, при persist поста Hibernate попытается persist тег — и выбросит `EntityExistsException` («detached entity passed to persist»). Нельзя persist уже существующую сущность. Решение: для существующих тегов использовать `merge()` или просто добавлять managed-экземпляры (загруженные в текущей сессии). Для `@ManyToMany` со справочными данными используйте только `CascadeType.MERGE`, но не `PERSIST`.
...
**↳ Ответ:** `orphanRemoval = true` предполагает эксклюзивное владение: дочерний объект принадлежит ровно одному родителю. Если дочерняя сущность убрана из коллекции одного родителя — Hibernate выполнит DELETE, даже если другой родитель всё ещё ссылается на неё. Это нарушение целостности: второй родитель будет ссылаться на несуществующую строку. Правило: `orphanRemoval = true` только когда гарантировано, что дочерний объект принадлежит ровно одному родителю — это агрегатная (композитная) связь, а не ссылка.
...
**↳ Ответ:** `List` в Hibernate — это `Bag`: при удалении одного элемента Hibernate выполняет DELETE всех записей и INSERT оставшихся — дорогостоящая операция. `Set` делает точечный DELETE. Также два одновременных JOIN FETCH с `List` вызывает `MultipleBagFetchException`, с `Set` — нет. Подвох: `Set` требует корректного `equals`/`hashCode` — если использовать `id`, transient-объект (id=null) не сможет корректно добавиться в Set. Правило: `hashCode()` = `getClass().hashCode()`, `equals` проверяет id с null-guard.
...
**↳ Ответ:** `@EqualsAndHashCode` по умолчанию включает все поля, в том числе lazy-коллекции — их вызов вне сессии вызывает `LazyInitializationException`. Если включить только `id`, transient-объект имеет id=null — все они "равны", что ломает HashSet. Лучшее решение для JPA-сущностей: не использовать Lombok для equals/hashCode, писать вручную: `hashCode` возвращает `getClass().hashCode()` (константный), `equals` сравнивает классы и id с null-guard.
...
**↳ Ответ:** (1) **Утечка внутренней модели** — клиент видит поля, которые не должен (пароли, внутренние статусы), и любое изменение схемы БД ломает API-контракт. (2) **LazyInitializationException** — при сериализации вне транзакции Hibernate пытается подгрузить lazy-поля на уже закрытой сессии. (3) **Производительность** — сериализуются лишние поля и вложенные объекты. (4) **Coupling** — контроллер жёстко связан с БД-моделью, усложняя эволюцию. DTO разрывает эту связь и решает все четыре проблемы.
...
**↳ Ответ:** Нативный SQL оправдан когда: нужны специфичные функции БД (`LATERAL JOIN`, `FILTER`, оконные функции с `PARTITION BY`), массовые операции через `INSERT INTO ... SELECT`, использование CTE (`WITH`), или SQL сгенерирован динамически внешним инструментом (jOOQ). Ограничения нативных запросов: не переносимы между БД, нет маппинга по именам полей Java (маппинг по позиции или `@SqlResultSetMapping`), нет поддержки полиморфных запросов по иерархии наследования. Результат нужно маппить в DTO вручную или через `@NamedNativeQuery`.
...
**↳ Ответ:** `Specification` — это обёртка над Criteria API, реализующая паттерн Specification: каждое условие — отдельный объект, они комбинируются через `and()`/`or()`/`not()`. Это чище и тестируемее, чем raw Criteria API. Querydsl лучше когда: нужны сложные JOIN с несколькими таблицами, типобезопасность без MetaModel-генерации, или хочется DSL-стиль близкий к SQL (`QOrder.order.status.eq(ACTIVE).and(...)`) — он читается как нативный запрос, в отличие от Criteria/Specification.
...
**↳ Ответ:** JOIN FETCH умножает строки (1 User × N Orders = N строк), а SQL LIMIT/OFFSET применяется к строкам, не к корневым сущностям. Hibernate предупреждает и делает пагинацию в памяти — грузит ВСЕ данные, потом обрезает. Правильный паттерн для коллекций с пагинацией: (1) сначала постранично получи id корневых сущностей (`SELECT u.id FROM User u ... LIMIT ? OFFSET ?`), (2) затем отдельным запросом с JOIN FETCH или `@BatchSize` подгрузи коллекции только для этих id.
...
**↳ Ответ:** DTO-проекция через JPQL `new` выражение: `SELECT new com.example.dto.UserOrderCountDto(u.id, u.email, COUNT(o)) FROM User u LEFT JOIN u.orders o GROUP BY u.id, u.email`. Это один SQL-запрос, нет N+1, нет загрузки лишних полей. Interface-based проекции тут хуже: вложенная агрегация (`COUNT`) в них неудобна, генерируемый SQL непредсказуем. Entity-проекция исключена — загрузит все поля плюс возможен N+1 при обращении к `orders.size()`.
...
**↳ Ответ:** `SINGLE_TABLE` — один SELECT по одной таблице, максимально быстро. `JOINED` — SELECT с JOIN всех дочерних таблиц к родительской: количество JOIN = количество подклассов, при глубокой иерархии из 10 подклассов — 10 LEFT OUTER JOIN. `TABLE_PER_CLASS` — UNION ALL всех таблиц подклассов: каждый подкласс отдельный SELECT + UNION — при 10 подклассах 10 запросов плюс объединение. Вывод: для частых полиморфных запросов выбирай SINGLE_TABLE; JOINED если важна нормализация при редких полиморфных запросах; TABLE_PER_CLASS избегай для полиморфных запросов.
