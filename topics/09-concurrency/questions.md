...
**↳ Ответ:** Да, `volatile long` и `volatile double` атомарны согласно JLS §17.7, даже на 32-битных JVM. Без `volatile`: запись `long`/`double` разбивается на две 32-битных операции — поток может увидеть «наполовину записанное» 64-битное значение. С `volatile` JMM обязывает JVM обеспечить атомарность записи/чтения. На практике 32-битные x86 реализуют это через `CMPXCHG8B` или аналогичные инструкции. Правило: для `long` и `double` в многопоточном коде всегда добавляйте `volatile` или используйте `AtomicLong`.
...
**↳ Ответ:** Нет, без синхронизации — не гарантировано. Если один поток создаёт объект и передаёт ссылку другому через обычное (non-volatile, non-synchronized) поле, другой поток может увидеть объект с частично инициализированными полями. Конструктор сам по себе не устанавливает happens-before. Безопасная публикация: через `volatile`-поле, через `final` поля (JLS §17.5 — final fields имеют special freeze action), через `synchronized`, через `AtomicReference`, или через статическую инициализацию. Правило: если передаёте объект между потоками — нужна одна из этих форм безопасной публикации.
...
**↳ Ответ:** Разные, хотя часто совпадают. Visibility problem — поток не видит актуальное значение, написанное другим потоком, из-за кэширования CPU/JIT. Достаточно `volatile` для одного поля. Race condition — корректность зависит от порядка операций: между read и write другой поток успевает изменить состояние. `volatile` не помогает — нужна атомарность составной операции (`synchronized` или `AtomicInteger`). Пример: `volatile boolean flag` — visibility достаточно. `volatile int counter; counter++` — visibility есть, но race condition сохраняется, потому что increment — три операции.
...
**↳ Ответ:** `notify()` будит произвольно одного потока из wait set; `notifyAll()` будит всех. Использовать `notify()` опасно, когда в wait set могут оказаться потоки, ожидающие разных условий: если разбудить «не того» — тот снова уйдёт в wait, а нужный поток так и останется заблокированным (missed signal). Безопасно использовать `notify()` только если: все потоки в wait set ждут одного и того же условия, и выполнение одного из них освобождает ровно один ресурс. Правило по умолчанию: `notifyAll()` — безопаснее, медленнее; `notify()` — быстрее, требует строгих гарантий.
...
**↳ Ответ:** JDK 15 отключил biased locking по умолчанию (JEP 374), JDK 18 окончательно удалил. Причины: biased locking стоило слишком дорого при revocation — когда появлялся второй поток, JVM должна была «остановить мир» (safepoint), снять bias, что дороже, чем выгода для однопоточных сценариев. Современные JVM используют lightweight locking через CAS в стеке потока (без OS) для умеренной конкуренции, и heavyweight locking через OS mutex для высокой — без biased locking. В Java 21+ работа над поддержкой виртуальных потоков сделала biased locking архаизмом.
...
**↳ Ответ:** Нет. `synchronized(null)` бросает `NullPointerException` в момент входа в блок. Это происходит потому, что монитор неотделим от объекта — без объекта нет монитора. Типичная ловушка: поле-локер инициализируется лениво или может быть `null` при неправильной инициализации. Правило: локер всегда должен быть `private final Object lock = new Object()` — инициализированным в декларации, чтобы исключить NPE и случайный обмен ссылкой.
...
**↳ Ответ:** Fair (честный) режим — поток получает lock в порядке FIFO-очереди: кто дольше ждёт, тот первым захватит. Unfair (нечестный, по умолчанию) — новый поток может «украсть» lock у ожидающих, если lock освободился именно в этот момент. Fair режим предотвращает starvation (вечное ожидание одного потока), но в 10-100 раз медленнее: каждый захват требует проверки очереди и передачи через OS scheduler. Unfair быстрее, потому что новый поток может взять lock без засыпания, используя cache locality. Правило: unfair по умолчанию; fair только если starvation — реальная проблема.
...
**↳ Ответ:** `LongAdder` предпочтительнее, когда много потоков часто инкрементируют один счётчик и редко читают итог — например, метрики запросов. Преимущество достигается striping: вместо одной переменной `LongAdder` содержит массив ячеек (Cell[]), каждый поток инкрементирует свою ячейку (определяется хэшом потока). При конкуренции на одной ячейке — поток переключается на другую. Итог `sum()` = сумма всех ячеек. Цена: `sum()` неточен в момент вызова (другие потоки могут инкрементировать параллельно), и потребляет больше памяти. Правило: `LongAdder` для write-heavy счётчиков; `AtomicLong` когда нужен точный снапшот или read-modify-write.
...
**↳ Ответ:** `AtomicStampedReference` хранит пару (reference, stamp) — ссылку и целочисленную версию-штамп. CAS теперь проверяет оба: и значение, и stamp. Поток B изменяет A→B→A? — stamp становится 1→2→3. Поток A делает CAS(ref=A, stamp=1, ...) — видит stamp=3, провалится. Недостатки: два CAS-а в одной операции (на 32-битных JVM реализуется через одну 64-битную CAS, но overhead выше); чуть сложнее в использовании; если stamp переполнится (`Integer.MAX_VALUE`) — риск ложного совпадения. Альтернатива: `AtomicMarkableReference` с boolean-меткой вместо счётчика.
...
**↳ Ответ:** `Integer` — иммутабельный класс, спроектированный без изменяемого состояния. Наследовать его и добавить мутабельность — нарушение Liskov Substitution Principle. Практически: `AtomicInteger` нельзя использовать там, где ожидается `Integer` или `Number` без явного вызова `intValue()`. Это предотвращает случайное смешение: `Map<String, Integer>` не принимает `AtomicInteger`, что является правильным ограничением — один — счётчик с многопоточной семантикой, другой — просто число. Правило: если метод принимает `Integer` — вы не случайно передадите `AtomicInteger` и не создадите иллюзию потокобезопасности там, где её нет.
...
**↳ Ответ:** `LongAdder` использует striped counter: массив `Cell[]`, где каждый `Cell` содержит одно `long` значение. Каждая `Cell` выровнена так, чтобы занимать отдельную кэш-линию (64 байта) с помощью аннотации `@sun.misc.Contended` — это предотвращает false sharing. Поток хэшируется (по ID или через probe value) к конкретной ячейке. При CAS-конфликте на ячейке поток перехэшируется к другой. Итог — разные потоки пишут в разные кэш-линии, нет конкуренции на шине памяти. `sum()` обходит все ячейки и суммирует.
...
**↳ Ответ:** CPU-bound: `corePoolSize` = число CPU ядер (`Runtime.getRuntime().availableProcessors()`), иногда +1. Больше ядер не поможет — потоки будут конкурировать за CPU, вызывая лишние context switch. IO-bound: формула Литтла — `corePoolSize = N_cpu * target_utilization * (1 + wait_time / compute_time)`. Если IO-запрос занимает 100ms и compute 10ms, ratio = 10, значит `corePoolSize = cores * 10`. На практике: IO-bound сервисы часто имеют пул 50-200 потоков. Но точная цифра — только нагрузочное тестирование. Правило: начните с `cores * 4` для IO, `cores` для CPU, потом нагрузочный тест.
...
**↳ Ответ:** `shutdownNow()` посылает `interrupt` всем активным потокам и возвращает список задач, которые были в очереди но ещё не начали выполняться — эти задачи просто выбрасываются (не запускаются). Задачи, уже выполняющиеся в потоках, получают interrupt, но остановятся только если их код обрабатывает `InterruptedException` или проверяет `Thread.currentThread().isInterrupted()`. Правило: если нужно корректно завершить текущие задачи — используй `shutdown()` + `awaitTermination()`; `shutdownNow()` — это экстренный стоп с потерей очереди.
...
**↳ Ответ:** `ExecutionException` — обёртка; реальное исключение внутри: `e.getCause()`. Паттерн: `catch (ExecutionException e) { throw new RuntimeException(e.getCause()); }` или, если нужно сохранить тип, `Throwables.throwIfUnchecked(e.getCause())` (Guava). Никогда не логируй и не глотай сам `ExecutionException` — пользователю нужен `cause`, а не обёртка.
...
**↳ Ответ:** `CallerRunsPolicy` — RejectedExecutionHandler, который при переполнении пула выполняет задачу прямо в вызывающем потоке (например, в HTTP-потоке Tomcat). Это замедляет производителя задач пропорционально загрузке потребителя — естественный backpressure без явного блокирования. Вызывающий поток занят выполнением задачи и не может отправить следующую, что снижает давление на пул. Альтернатива `AbortPolicy` просто бросает исключение — без backpressure, с потерей задачи.
...
**↳ Ответ:** `ManagedBlocker` — интерфейс, который сигнализирует ForkJoinPool: «я собираюсь заблокироваться». ForkJoinPool при этом может создать дополнительный поток-заменитель, чтобы поддержать нужный уровень параллелизма. Без него заблокировавший поток просто простаивает, и parallelism падает. Используется редко (только при неизбежной блокировке в FJP); правильнее — выносить IO-операции в отдельный executor.
...
**↳ Ответ:** `jstack <pid>` выводит thread dump всех потоков. При deadlock в конце вывода будет секция `Found one Java-level deadlock:` с именами потоков, объектами-мониторами которые они держат и ждут. Алгоритм: 1) найти потоки в состоянии `BLOCKED`; 2) посмотреть на какой монитор они ждут (`waiting to lock <0x...>`); 3) найти кто держит этот монитор (`locked <0x...>`). Для автоматической проверки в коде: `ManagementFactory.getThreadMXBean().findDeadlockedThreads()`.
...
**↳ Ответ:** Проще всего устранить **циклическое ожидание** — установить глобальный порядок захвата ресурсов и всегда следовать ему. Например, при переводе между счётами всегда блокировать счёт с меньшим `id` первым. Это не требует изменения механики блокировки, только дисциплины в коде. Взаимное исключение устранить нельзя (иначе нарушается корректность), No Preemption — сложно (нужна поддержка ОС), Hold & Wait — сложно (нужно брать все ресурсы атомарно).
...
**↳ Ответ:** При deadlock потоки **не двигаются** — они в состоянии `BLOCKED` или `WAITING`, CPU 0%. При livelock потоки **активны** — постоянно меняют состояние, реагируют друг на друга, но не продвигаются вперёд; CPU близко к 100%. В thread dump livelock плохо виден: потоки будут в состоянии `RUNNABLE`, но трассировки будут показывать одни и те же методы retry-цикла. Диагностика — наблюдение во времени: потоки активны, но прогресса нет.
...
**↳ Ответ:** `computeIfAbsent` блокирует только конкретный bucket (сегмент) карты на время вычисления. Если два разных ключа попадают в один bucket — они выполняются последовательно. Но главная проблема: если вычисление (`loader`) само по себе долгое (HTTP-запрос, DB-запрос), другие потоки с тем же ключом будут ждать внутри lock'а bucket'а. При этом в Java 8 был баг: рекурсивный `computeIfAbsent` на той же карте мог зависнуть. Для по-настоящему дорогой инициализации лучше использовать double-checked locking с `volatile` или `ConcurrentHashMap` с `putIfAbsent` + явным отдельным lock'ом.
...
**↳ Ответ:** `StampedLock` добавляет третий режим — оптимистичное чтение (`tryOptimisticRead()`): читаем без блокировки, потом проверяем `validate(stamp)` — не было ли записи. Если запись была — деградируем до полного readLock. `ReentrantReadWriteLock` всегда блокирует при чтении (несовместимо с writeLock). Оптимистичное чтение оправдано, когда записи редки (конфигурация, справочники), а чтений много — мы избегаем блокировки в ~99% случаев. Ограничение: `StampedLock` **не реентрантен**, что нужно помнить при сложных вызовах.
...
**↳ Ответ:** Pinning — состояние, когда виртуальный поток не может unmount'иться с несущего OS-потока: происходит при блокировке внутри `synchronized` блока (Java 21) или внутри нативного (JNI) метода. Пока VT «приколот» — carrier OS-поток тоже заблокирован, масштабируемость теряется. Обнаружение: JVM флаг `-Djdk.tracePinnedThreads=full` выводит стек при каждом pinning в stderr. Через JFR: событие `jdk.VirtualThreadPinned` — записывает стек и длительность. Решение: заменить `synchronized` на `ReentrantLock` в горячих путях.
...
**↳ Ответ:** `ThreadLocal` работает с VT корректно — у каждого виртуального потока своя копия. Проблема в другом: при модели «один VT на запрос» создаются миллионы VT, и если каждый держит большой объект в ThreadLocal (например, кэш), это приводит к memory pressure. `ScopedValue` (Java 21+, preview) — иммутабельная, привязанная к лексическому блоку переменная: автоматически освобождается при выходе из блока, не требует `remove()`, не наследуется случайно дочерними потоками. Правило: `ThreadLocal` — для изменяемых per-thread данных; `ScopedValue` — для передачи контекста (userId, requestId) по стеку вызовов.
...
**↳ Ответ:** `ThreadLocal` в VT работает как обычно, но создаёт проблемы: 1) утечки памяти — если не вызвать `remove()`, объект висит до GC виртуального потока; 2) неожиданное наследование через `InheritableThreadLocal` — дочерний VT получает копию родительского контекста, что при миллионах VT может быть неожиданным. `ScopedValue` решает это: значение доступно только в рамках `ScopedValue.where(...).run(...)` блока, автоматически освобождается, явно передаётся в дочерние scope — нет случайного наследования.
...
**↳ Ответ:** Если родительский VT прерван во время `scope.join()`, метод бросает `InterruptedException`. При выходе из try-with-resources блока (нормальном или через исключение) `scope.close()` автоматически отменяет все незавершённые подзадачи и ждёт их завершения. Это гарантирует что при прерывании родителя ни одна дочерняя задача не «убегает» в фоне — ключевое свойство structured concurrency.
...
**↳ Ответ:** Нет. При повторном вызове `start()` на уже запущенном или завершённом `Thread` JVM бросает `IllegalThreadStateException`. Поток — одноразовый объект: после завершения его нельзя перезапустить. Правило: если нужна повторяемость — используй `Runnable` + новый `Thread` каждый раз, или лучше `ExecutorService.submit()`.
...
**↳ Ответ:** `CompletableFuture` добавляет к `Future`: цепочки (`thenApply`, `thenCompose`), комбинирование (`allOf`, `anyOf`), callback-и (`thenAccept`), явное завершение (`complete(value)`), обработку ошибок (`exceptionally`). `get()` бросает checked `ExecutionException` + `InterruptedException` — нужен try-catch. `join()` бросает unchecked `CompletionException` — удобнее в stream/lambda, но скрывает тип исключения. Правило: в сигнатурах методов с throws — `get()`; внутри `thenApply`/stream — `join()`.
